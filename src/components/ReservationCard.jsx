            ...
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                <div className="space-y-1">
                    <div className="flex flex-wrap items-center gap-2">
                        <h3 className="text-sm font-semibold text-slate-900 sm:text-base">{sala}</h3>

                        <span
                            className={[
                                "rounded-full px-2 py-0.5 text-[11px] font-medium",
                                isAdmin ? "bg-green-50 text-green-700" : "bg-blue-50 text-blue-700",
                            ].join(" ")}
                        >
              {edificio}
            </span>
                    ...
                    <p className="text-xs text-slate-600 sm:text-sm">{fecha} · {turno}</p>

                    {organizador && (
                        <p className="text-[11px] text-slate-500">
                            Organiza: <span className="font-medium text-slate-800">{organizador?.nombre ? `${organizador.nombre} ${organizador.apellido}` : organizador?.ci ?? organizador}</span>
                        </p>
                    )}

                    <div className="text-[11px] text-slate-500 flex flex-wrap gap-2">
            <span>
              Participantes: <strong className="text-slate-800">{participantes ?? 0}/{capacidad ?? 0}</strong>
            </span>

                        <span className="text-slate-400">·</span>

                        <span className={["font-medium", stateColorClass].join(" ")}>{estado}</span>
                    </div>
                </div>
              ...
              hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-green-600
            "
                    >
                        Ver
                    </button>
                    ...
                    {isAdmin ? (
                        <>
                            <button
                                ...
                                className="
                  rounded-xl border border-green-700 px-4 py-2 text-sm font-semibold text-green-700
                  hover:bg-green-50 focus:outline-none focus:ring-2 focus:ring-green-600
                "
                            >
                                Editar
                            </button>
                            ...
                            <button
                                ...
                                className="
      rounded-xl bg-green-700 px-4 py-2 text-sm font-semibold text-white
